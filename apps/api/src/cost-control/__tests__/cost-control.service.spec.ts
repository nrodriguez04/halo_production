...
describe('IntegrationCostControlService', () => {
  ...
  it('blocks after exhausting a cyclic fallback chain instead of recursing forever', async () => {
    const overCap = {
      id: 'b_1',
      scope: 'provider',
    ...
    budgets.findApplicable.mockResolvedValue([overCap]);
    budgets.findOverHardCap
      .mockReturnValueOnce(overCap)
      .mockReturnValueOnce({ ...overCap, scopeRef: 'datazapp' })
      .mockReturnValueOnce({ ...overCap, scopeRef: 'propertyradar' });
    ...
    expect(exec).not.toHaveBeenCalled();
    expect(budgets.findOverHardCap).toHaveBeenCalledTimes(3);
  });
});
