...
describe('IntegrationCostControlService', () => {
  ...
  beforeEach(async () => {
    ...
    prisma.integrationProvider.findUnique.mockImplementation(async (args: any) => {
      ...
      if (args.where.key === 'resend') return { id: 'p_resend', key: 'resend', enabled: true, rateLimitPerMin: 60 };
  ...
  it('returns DOWNGRADE_PROVIDER when over hard cap and provider has a supported fallback', async () => {
    budgets.findApplicable.mockResolvedValueOnce([
      { id: 'b_1', scope: 'provider', scopeRef: 'resend', period: 'month', hardCapUsd: 100, softCapUsd: 80, currentSpendUsd: 99.95, enabled: true },
    ]);
    budgets.findOverHardCap.mockReturnValueOnce({ id: 'b_1', scope: 'provider', scopeRef: 'resend', period: 'month', hardCapUsd: 100, softCapUsd: 80, currentSpendUsd: 99.95, enabled: true });
    const decision = await service.preflight({
      ...baseIntent(),
      provider: 'resend',
      action: 'send_email',
  ...
  it('blocks over budget when a provider has no safe cross-provider fallback', async () => {
    budgets.findApplicable.mockResolvedValueOnce([
  ...
  it('blocks over budget after every supported fallback has already been tried', async () => {
    budgets.findApplicable.mockResolvedValueOnce([
      { id: 'b_1', scope: 'provider', scopeRef: 'resend', period: 'month', hardCapUsd: 100, softCapUsd: 80, currentSpendUsd: 99.95, enabled: true },
    ]);
    budgets.findOverHardCap.mockReturnValueOnce({ id: 'b_1', scope: 'provider', scopeRef: 'resend', period: 'month', hardCapUsd: 100, softCapUsd: 80, currentSpendUsd: 99.95, enabled: true });
